...
def flood_fill(screen, sr, sc, new_color, old_color, view):
    ...
    if screen[sr][sc] == old_color:
        ...
        view.addstr(0, 0, ''.join(str(ele) for sub in screen for ele in sub))
        view.refresh()

        time.sleep(.1)   # Slow down traversal
...
if __name__ == '__main__':
    ...
    sr = 11                      # Color change starting coords
    sc = 45
    new_color = 2               # Color to be changed to

    # Create Screen Buffer
    screen = [[random.choice([0,0,0,1]) for x in range(nScreenWidth)] for y in range(nScreenHeight)]
